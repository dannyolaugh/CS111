 ...
 *ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) check
  ...
  while (r == 0 && ok_so_far >= 0 && f_pos >= 2 && f_pos < dir_oi->oi_size) 
    {
      ...
      if(od->od_ino  && entry_oi && entry_oi->oi_nlink)
	{
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
  ...
  if (entry_off == dir_oi->oi_size) {
    eprintk("<1>ospfs_unlink should not fail!\n");
    return -ENOENT;
  }
...
allocate_block(void)
{
  uint32_t free_place = -1;
  uint32_t block_pos = OSPFS_FREEMAP_BLK + 1;
  while(block_pos <= (ospfs_super->os_nblocks - 1))
    {
      if(bitvector_test(ospfs_block(OSPFS_FREEMAP_BLK) ,block_pos) == 1)
        {
	  free_place = block_pos;
	  break;
        }
      block_pos++;
    }
  if(free_place == -1)
    return 0;
  bitvector_clear(ospfs_block(OSPFS_FREEMAP_BLK), free_place);
  return free_place;
}
...
static void 
zero_b(uint32_t blockno)
{
  char *a = (char*)ospfs_block(blockno);
  int l;
  for(l=0; l < OSPFS_BLKSIZE; l++){
    a[l]=0;
  }
  return;
}
...
add_block(ospfs_inode_t *oi)
{
  ...
  uint32_t* indirect_ptr2;
  uint32_t indirect_data, indirect_data2;
  
  if(n >= OSPFS_MAXFILEBLKS)
    return -ENOSPC;
  
  if((blockno = allocate_block()) == 0)
    return -ENOSPC;
  zero_b(blockno);

  index_type = (int32_t)indir_index(n);
  if(index_type == -1)//direct block
    {
      oi->oi_direct[n] = blockno;
    }
  else if(index_type == 0) //indirect block
    {
      if(oi->oi_indirect)
        {
	  //indirect block exsists, add this block
	  indirect_ptr = ospfs_block(oi->oi_indirect);
	  indirect_ptr[direct_index(n)] = blockno;
        }
      else
        {
	  //make new indirect block
	  if((indirect_data = allocate_block()) == 0)
            {
	      free_block(blockno);
	      return -ENOSPC;
            }
	  
	  zero_b(indirect_data);
	 
	  oi->oi_indirect = indirect_data;
  ...
  else//double indirect block
    {
      if(oi->oi_indirect2)
        {
	  //doubly indirect block already made
	  indirect_ptr2 = ospfs_block(oi->oi_indirect2);
	  if(indirect_ptr2[indir_index(n)])
            {
	      //indirect block exsists, add this block
	      indirect_ptr = ospfs_block(indirect_ptr2[indir_index(n)]);
	      indirect_ptr[direct_index(n)] = blockno;
            }
	  else
            {
	      ...
	      indirect_ptr2[indir_index(n)] = indirect_data;
	      indirect_ptr = ospfs_block(indirect_ptr2[indir_index(n)]);
	      indirect_ptr[direct_index(n)] = blockno;
            }
        }
        ...
        {
	  //make doubly indirect block
	  if((indirect_data2 = allocate_block()) == 0)
            {
	      free_block(blockno);
	      return -ENOSPC;
            }
	  zero_b(indirect_data2);
	  
	  oi->oi_indirect2 = indirect_data2;
  ...
  if(index_type == -1)//direct block
	...
	return -EIO;
      ...
      if(n > OSPFS_NDIRECT + 1)
        {
	  uint32_t* indirect_ptr = ospfs_block(oi->oi_indirect);
	  free_block(indirect_ptr[direct_index(n)]);
	  indirect_ptr[direct_index(n)] = 0;
        }
      else
        {
	  uint32_t* indirect_ptr = ospfs_block(oi->oi_indirect);
    ...
    {
      uint32_t* indirect_ptr;
      uint32_t* indirect_ptr2 = ospfs_block(oi->oi_indirect2);
      if(oi->oi_indirect2 <= 0)
	return -EIO;
      
      in_index = indir_index(n);
      indirect_ptr = ospfs_block(indirect_ptr2[n]);
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
  ...
  if((ret = change_size(dir_oi, dir_oi->oi_size + OSPFS_BLKSIZE))!= 0)
    return ERR_PTR(ret);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry)
{
  ...
  strncpy(new_dir->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);
  new_dir->od_name[dst_dentry->d_name.len] = 0;
  
  ino = ospfs_inode(src_dentry->d_inode->i_ino);
  ino->oi_nlink++;
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
  ...
  //find free inode
  first = ospfs_super->os_firstinob;
  while(first < ospfs_super->os_ninodes)
    {
      oi = ospfs_inode(first);
      if(oi->oi_nlink == 0)
	break;
      first++;
    }
  
  //if full return ENOSPC
  if(first >= ospfs_super->os_ninodes)
    return -ENOSPC;
  
  entry_ino = first;
  
  //set up direntry
  new_dir->od_ino = first;
  for(j = 0; j < dentry->d_name.len; j++)
    {
      new_dir->od_name[j] = dentry->d_name.name[j];
    }
  new_dir->od_name[j] = 0;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
  ...
  while(first < ospfs_super->os_ninodes)
    {
      ois = (ospfs_symlink_inode_t*)ospfs_inode(first);
      if(ois->oi_nlink == 0)
	break;
      first++;
    }
  //actually found an inode
  if(first >= ospfs_super->os_ninodes)
    return -ENOSPC;
  ...
  //set up the symlink
  ois->oi_size = strlen(symname);
  ois->oi_ftype = OSPFS_FTYPE_SYMLINK;
  ois->oi_nlink = 1;
  strncpy(ois->oi_symlink, symname, ois->oi_size);
  ois->oi_symlink[ois->oi_size] = 0;
...
MODULE_AUTHOR("John Storch and Daniel O'Laughlin");
