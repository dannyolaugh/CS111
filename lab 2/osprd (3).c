...
struct read_pid_list{
  pid_t pid;
  struct read_pid_list *next_pid;
};
typedef struct read_pid_list *pid_list;
...
void set_read_list(pid_list now, pid_list last, osprd_info_t* d)
{
  while(now != NULL)
    {
      last = now;
  ...
  if(last == NULL)
    {
      d->read_pids = kmalloc(sizeof(pid_list), GFP_ATOMIC);
      d->read_pids->pid = current->pid;
      d->read_pids->next_pid = NULL;
    }
  else
    {
      last->next_pid = kmalloc(sizeof(pid_list), GFP_ATOMIC);
      last->next_pid->pid = current->pid;
      last->next_pid->next_pid = NULL;
    }
}

void release_read_lock(pid_list now, pid_list last)
{
  while(now != NULL)
    {
